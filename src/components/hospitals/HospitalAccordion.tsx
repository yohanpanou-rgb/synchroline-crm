...
import { removeDoctorFromInstitution, setInstitutionReps, deleteInstitution } from "@/app/(app)/hospitals/actions";
...
export function HospitalAccordion({
  ...
  function handleDelete(institutionId: string, institutionName: string) {
    if (!window.confirm(`Διαγραφή του νοσοκομείου "${institutionName}"; Δεν αναιρείται.`)) return;
    setDeleteError(null);
    startTransition(async () => {
      const result = await deleteInstitution(institutionId, institutionName);
            ...
            <div className="flex w-full items-center justify-between px-4 py-3">
              ...
              {manager && (
                <button
                  ...
                  className="ml-3 shrink-0 rounded-lg px-2 py-1 text-xs font-medium text-danger hover:bg-danger/10 disabled:opacity-50"
                >
                  Διαγραφή
                </button>
              )}
